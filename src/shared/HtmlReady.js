 ...
 *    - basic sanitization?
 *  2. next.vox.community Rendering - add in proprietary next.vox.community functions/links
        ...
        if (state.mutate) {
            ...
            if (
                (url.indexOf('#') !== 0 && // Allow in-page links
                    (child.textContent.match(/(www\.)?vox\.community/i) &&
                        !url.match(
                            /https?:\/\/(.*@)?(www\.)?vox\.community/i
                        ))) ||
